...
const HabitDetails = () => {
  ...
  useEffect(() => {
    axios
      .get(`https://habit-tarcker-server.vercel.app/details/${id}`)
      .then((res) => {
  ...
  const handleMarkComplete = async () => {
    ...
    try {
      ...
      const res = await axios.put(
        `https://habit-tarcker-server.vercel.app/habits/complete/${id}`
      );
  ...
  if (!habit) return <p className="text-white text-center mt-20">Loading...</p>;
